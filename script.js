document.addEventListener('DOMContentLoaded', () => {
    ...
    function showChat(name) {
        ...
        chatContainer.style.display = 'flex'; // Use flex for chat layout
        welcomeMessage.textContent = `Welcome, ${name}!`;
        connectToChat(name); // Connect to chat after showing the UI
    }
    ...
    function showLogin() {
        ...
        updateFormUI();
        if (socket) {
            socket.disconnect(); // Disconnect from chat if going back to login
        }
    }
    
    // (updateFormUI remains the same)
    ...
    // --- NEW Real-Time Chat Functions ---
    function connectToChat(name) {
        socket = io(API_URL); // Establish connection

        // Listen for incoming messages from the server
        socket.on('chat message', (msg) => {
            appendMessage(msg.name, msg.text, 'received');
        });
    }
    ...
    function sendMessage(name, text) {
        if (text && socket) {
            ...
            appendMessage('You', text, 'sent'); // Display your own message immediately
    ...
    chatForm.addEventListener('submit', (e) => {
        e.preventDefault();
        const currentUserName = localStorage.getItem('currentUserName');
        sendMessage(currentUserName, messageInput.value);
    });
